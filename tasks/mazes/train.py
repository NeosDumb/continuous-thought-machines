...
def parse_args():
    ...
    )  # Default changed from original script
    # CTM / LSTM specific
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Mazes between 0 and 1 = False. Between -1 and 1 = True. Legacy checkpoints use 0 and 1.",
    )
    parser.add_argument(
        "--batch_size", type=int, default=16, help="Batch size for training."
    )  # Default changed
    parser.add_argument(
        "--batch_size_test", type=int, default=64, help="Batch size for testing."
    )  # Default changed
    parser.add_argument(
    ...
    )  # Renamed from num_workers, kept default
    parser.add_argument(
        "--gradient_clipping",
        type=float,
        default=-1,
    ...
    parser.add_argument(
        ...
        help="Dataset to use.",
        choices=["mazes-medium", "mazes-large", "mazes-small"],
    )
    parser.add_argument(
        "--data_root", type=str, default="data/mazes", help="Data root."
    )
...
if __name__ == "__main__":
    ...
    train_data = MazeImageFolder(
        ...
        expand_range=args.expand_range,
    )
    ...
    num_workers_test = 4  # Defaulting to 1, can be changed
        ...
        for bi in range(start_iter, args.training_iterations):
            ...
            if bi % args.track_every == 0 and (bi != 0 or args.reload_model_only):
                ...
                with torch.inference_mode():  # Use inference mode for tracking
                    ...
                    with tqdm(
                        ...
                        for inferi, (inputs, targets) in enumerate(loader):
                            ...
                            if args.model == "ctm":
                                ...
                                predictions = predictions_raw.reshape(
                                    predictions_raw.size(0),
                                    -1,
                                    5,
                                    predictions_raw.size(-1),
                                )  # B,S,C,T
                    ...
                    all_predictions = (
                        torch.cat(all_predictions_list).cpu().numpy()
                    )  # N, S, T or N, S
                    ...
                    if args.model in CTM_LSTM_MODELS:
                        ...
                        train_accuracies.append(
                            np.mean(all_predictions == all_targets, axis=0)
                        )  # Mean over N -> (S,)
                    ...
                    train_acc_plot = [
                        np.mean(acc_s) for acc_s in train_accuracies
                    ]  # List[Scalar] or List[Scalar] after mean
                    test_acc_plot = [
                        np.mean(acc_s) for acc_s in test_accuracies
                    ]  # List[Scalar] or List[Scalar] after mean

                    axacc_train.plot(
                        iters, train_acc_plot, "g-", alpha=0.5, label="Avg Step Acc"
                    )
                    axacc_test.plot(
                        iters, test_acc_plot, "g-", alpha=0.5, label="Avg Step Acc"
                    )
                    ...
                    if args.model in CTM_LSTM_MODELS:
                        ...
                        predictions_viz = predictions_viz_raw.reshape(
                            predictions_viz_raw.size(0),
                            -1,
                            5,
                            predictions_viz_raw.size(-1),
                        )  # B, S, C, T
