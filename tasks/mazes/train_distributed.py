...
warnings.filterwarnings(
    "ignore", "Corrupt EXIF data", UserWarning, r"^PIL\.TiffImagePlugin$"
)
warnings.filterwarnings(
    "ignore", "UserWarning: Metadata Warning", UserWarning, r"^PIL\.TiffImagePlugin$"
)
warnings.filterwarnings(
    "ignore", "UserWarning: Truncated File Read", UserWarning, r"^PIL\.TiffImagePlugin$"
)
...
def parse_args():
    ...
    parser.add_argument(
        "--heads", type=int, default=8, help="Number of attention heads (CTM, LSTM)."
    )
    parser.add_argument(
        "--iterations",
        type=int,
    ...
    # Task Specific Args
    parser.add_argument(
        "--maze_route_length", type=int, default=100, help="Length to truncate targets."
    )
    parser.add_argument(
        "--cirriculum_lookahead",
        type=int,
        default=5,
        help="How far to look ahead for cirriculum.",
    )

    # Training
    parser.add_argument(
        "--batch_size", type=int, default=16, help="Batch size for training (per GPU)."
    )
    parser.add_argument(
        "--batch_size_test",
        type=int,
        default=64,
        help="Batch size for testing (per GPU).",
    )
    parser.add_argument(
        "--lr", type=float, default=1e-4, help="Learning rate for the model."
    )
    parser.add_argument(
        "--training_iterations",
        type=int,
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Try to compile model components.",
    )
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="mazes-medium",
        help="Dataset to use.",
        choices=["mazes-medium", "mazes-large"],
    )
    parser.add_argument(
        "--save_every",
        type=int,
    ...
    )  # Default False based on user edit
    parser.add_argument(
        "--strict_reload",
        action=argparse.BooleanOptionalAction,
        default=False,
    ...
    parser.add_argument(
        "--ignore_metrics_when_reloading",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Ignore metrics when reloading (for debugging)?",
    )
...
if __name__ == "__main__":
    ...
    if is_main_process(rank):
        print(
            f"Main process (Rank {rank}): Using device {device}. World size: {world_size}. Model: {args.model}"
        )
    ...
    try:
        ...
        h_w = (
            39 if args.dataset in ["mazes-small", "mazes-medium"] else 99
        )  # Example dimensions
    ...
    if is_main_process(rank):
        print(
            f"Total params: {sum(p.numel() for p in model_base.parameters() if p.requires_grad)}"
        )
    ...
    train_data = MazeImageFolder(
        root=f"{data_root}/train/",
        which_set="train",
        maze_route_length=args.maze_route_length,
    )
    ...
    train_accuracies_most_certain, test_accuracies_most_certain = (
        [],
        [],
    )  # Avg Step Acc @ Certain tick (scalar list)
    ...
    scaler = torch.amp.GradScaler(
        "cuda" if device.type == "cuda" else "cpu", enabled=args.use_amp
    )
    ...
    if args.reload:
        ...
        if os.path.isfile(chkpt_path):
            ...
            checkpoint = torch.load(
                chkpt_path, map_location=map_location, weights_only=True
            )
            ...
            if has_module_prefix and not is_wrapped:
                ...
                load_result = model_to_load.load_state_dict(
                    state_dict, strict=args.strict_reload
                )
                print(
            ...
            if not args.reload_model_only:
                print(
                    f"Rank {rank}: Reloading optimizer, scheduler, scaler, iteration."
                )
                ...
                if is_main_process(rank) and not args.ignore_metrics_when_reloading:
                    ...
                    train_accuracies = checkpoint[
                        "train_accuracies"
                    ]  # Reloading simplified avg step acc list
                    ...
                    train_accuracies_most_certain_permaze = checkpoint[
                        "train_accuracies_most_certain_permaze"
                    ]
                    test_accuracies_most_certain_permaze = checkpoint[
            ...
            if (
                ...
                and not args.reload_model_only
            ):
    ...
    if args.do_compile:
        ...
        if hasattr(model_to_compile, "backbone"):
            model_to_compile.backbone = torch.compile(
                model_to_compile.backbone, mode="reduce-overhead", fullgraph=True
            )
        if args.model == "ctm":
            model_to_compile.synapses = torch.compile(
                model_to_compile.synapses, mode="reduce-overhead", fullgraph=True
            )
        if world_size > 1:
            dist.barrier()
        if is_main_process(rank):
    ...
    for bi in range(start_iter, args.training_iterations):
        ...
        if bi % args.track_every == 0 and (bi != 0 or args.reload_model_only):
            ...
            with torch.inference_mode():
                ...
                total_train_correct_certain = torch.tensor(
                    0.0, device=device
                )  # Sum correct steps @ certain tick
                total_train_mazes_solved = torch.tensor(
                    0.0, device=device
                )  # Sum solved mazes @ certain tick
                total_train_steps = torch.tensor(
                    0.0, device=device
                )  # Total steps evaluated (B * S)
                total_train_mazes = torch.tensor(
                    0.0, device=device
                )  # Total mazes evaluated (B)

                # TRAIN METRICS
                train_eval_sampler = DistributedSampler(
                    train_data, num_replicas=world_size, rank=rank, shuffle=False
                )
                ...
                pbar_inner_desc = (
                    "Eval Train (Rank 0)" if is_main_process(rank) else None
                )
                with tqdm(
                    total=len(train_eval_loader),
                    desc=pbar_inner_desc,
                    leave=False,
                    position=1,
                    dynamic_ncols=True,
                    disable=not is_main_process(rank),
                    ...
                    for inferi, (inputs, targets) in enumerate(train_eval_loader):
                        ...
                        if args.model == "ctm":
                            ...
                            predictions = predictions_raw.reshape(
                                batch_size, -1, 5, predictions_raw.size(-1)
                            )  # B,S,C,T
                        ...
                        total_train_correct_certain += (
                            correct_steps.sum()
                        )  # Sum correct steps across batch
                        total_train_mazes_solved += correct_steps.all(
                            dim=-1
                        ).sum()  # Sum mazes where all steps are correct
                ...
                if is_main_process(rank) and total_train_mazes > 0:
                    avg_train_loss = (
                        total_train_loss.item() / total_train_mazes.item()
                    )  # Avg loss per maze/sample
                    avg_train_acc_step = (
                        total_train_correct_certain.item() / total_train_steps.item()
                    )  # Avg correct step %
                    avg_train_acc_maze = (
                        total_train_mazes_solved.item() / total_train_mazes.item()
                ...
                pbar_inner_desc = (
                    "Eval Test (Rank 0)" if is_main_process(rank) else None
                )
                with tqdm(
                ...
                if is_main_process(rank) and total_test_mazes > 0:
                    ...
                    avg_test_acc_step = (
                        total_test_correct_certain.item() / total_test_steps.item()
                    )
                    avg_test_acc_maze = (
                        total_test_mazes_solved.item() / total_test_mazes.item()
                    )
                    ...
                    axacc_train.plot(
                        ...
                        "k-",
                        alpha=0.7,
                        label=f"Avg Step Acc ({train_accuracies_most_certain[-1]:.3f})",
                    )
                    axacc_test.plot(
                        iters,
                        test_accuracies_most_certain,
                        "k-",
                        alpha=0.7,
                        label=f"Avg Step Acc ({test_accuracies_most_certain[-1]:.3f})",
                    )
                    ...
                    axacc_train.plot(
                        ...
                        "r-",
                        alpha=0.6,
                        label=f"Full Maze Acc ({train_accuracies_most_certain_permaze[-1]:.3f})",
                    )
                    axacc_test.plot(
                        iters,
                        test_accuracies_most_certain_permaze,
                        "r-",
                        alpha=0.6,
                        label=f"Full Maze Acc ({test_accuracies_most_certain_permaze[-1]:.3f})",
                    )
                    ...
                    axloss.plot(
                        ...
                        linewidth=1,
                        alpha=0.8,
                        label=f"Train (Agg): {train_losses[-1]:.4f}",
                    )
                    axloss.plot(
                        iters,
                        test_losses,
                        ...
                        linewidth=1,
                        alpha=0.8,
                        label=f"Test (Agg): {test_losses[-1]:.4f}",
                    )
                ...
                if is_main_process(rank) and args.model in CTM_LSTM_MODELS:
                    # try:
                    ...
                    att_shape = (
                        model.module.kv_features.shape[2],
                        model.module.kv_features.shape[3],
                    )
        ...
        if is_main_process(rank):
            ...
            if args.model in CTM_LSTM_MODELS or torch.is_tensor(where_most_certain):
