...
warnings.filterwarnings(
    "ignore",
    message="UserWarning: Metadata Warning, tag 274 had too many entries: 4, expected 1",
)
warnings.filterwarnings(
    "ignore",
    "Corrupt EXIF data",
...
def parse_args():
    ...
    parser.add_argument(
        "--data_root", type=str, default="data/", help="Where to save dataset."
    )
    parser.add_argument(
        "--save_every",
        type=int,
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Ignore metrics when reloading?",
    )
    ...
    parser.add_argument(
        ...
        default=[0],
        nargs="+",
        help="Which indices in test data to plot?",
    )  # Defaulted to 0
...
if __name__ == "__main__":
    ...
    if is_main_process(rank):
        ...
        with open(f"{args.log_dir}/args.txt", "w") as f:
            print(args, file=f)
    if world_size > 1:
        dist.barrier()  # Sync after rank 0 setup
    ...
    )  # No shuffle needed for test; consistent
    ...
    trainloader = torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        drop_last=True,
    )  # drop_last=True often used in DDP
    ...
    train_accuracies_most_certain = (
        [] if args.model in CTM_LSTM_MODELS else None
    )  # Scalar accuracy list
    test_accuracies_most_certain = (
        [] if args.model in CTM_LSTM_MODELS else None
    )  # Scalar accuracy list
    train_accuracies_standard = (
        [] if args.model == "ff" else None
    )  # Standard accuracy list for FF
    ...
    if args.reload:
        ...
        if os.path.isfile(chkpt_path):
            ...
            if not args.reload_model_only:
                ...
                if is_main_process(rank) and not args.ignore_metrics_when_reloading:
                    ...
                    if args.model in CTM_LSTM_MODELS:
                        ...
                        train_accuracies_standard = checkpoint[
                            "train_accuracies_standard"
                        ]
                        test_accuracies_standard = checkpoint[
                            "test_accuracies_standard"
                        ]
            ...
            if (
                ...
                print(
                    f"Rank {rank}: Loading RNG states (may need DDP adaptation for full reproducibility)."
                )
                torch.set_rng_state(
    ...
    if args.do_compile:
        ...
        if world_size > 1:
            dist.barrier()  # Sync after compilation
        if is_main_process(rank):
    ...
    for bi in range(start_iter, args.training_iterations):
        ...
        if bi % args.track_every == 0 and (bi != 0 or args.reload_model_only):
            ...
            with torch.inference_mode():
                ...
                if is_main_process(rank) and total_train_samples > 0:
                    avg_train_loss = (
                        total_train_loss.item() / total_train_samples.item()
                    )
                ...
                with tqdm(
                    ...
                    for inferi, (inputs, targets) in enumerate(
                        testloader
                    ):  # Testloader already uses sampler
                        inputs = inputs.to(device, non_blocking=True)
                        targets = targets.to(device, non_blocking=True)
                ...
                if is_main_process(rank) and total_test_samples > 0:
                    ...
                    if args.model in CTM_LSTM_MODELS:
                        ...
                        avg_test_acc_standard = (
                            total_test_correct_standard.item()
                            / total_test_samples.item()
                        )
                    ...
                    print(
                        f"Iter {bi} Test Metrics (Agg): Loss={avg_test_loss:.4f}, Acc={acc_val:.4f}\n"
                    )
                    ...
                    if args.model in CTM_LSTM_MODELS:
                        axacc_train.plot(
                            ...
                            "k-",
                            alpha=0.9,
                            label=f"Most certain ({train_accuracies_most_certain[-1]:.3f})",
                        )
                        axacc_test.plot(
                            iters,
                            test_accuracies_most_certain,
                        ...
                        axacc_train.plot(
                            ...
                            "k-",
                            alpha=0.9,
                            label=f"Standard Acc ({train_accuracies_standard[-1]:.3f})",
                        )
                        axacc_test.plot(
                            iters,
                            test_accuracies_standard,
                    ...
                    if args.dataset == "imagenet":
                        ...
                        if (
                            args.model in CTM_LSTM_MODELS
                            and len(train_accuracies_most_certain) > 0
                            and np.any(np.array(train_accuracies_most_certain) > 0.4)
                        ):
                            train_ylim_set = True
                            axacc_train.set_ylim([0.4, 1])
                        if (
                            args.model == "ff"
                            and len(train_accuracies_standard) > 0
                            and np.any(np.array(train_accuracies_standard) > 0.4)
                        ):
                            train_ylim_set = True
                            axacc_train.set_ylim([0.4, 1])
                        ...
                        if (
                            args.model in CTM_LSTM_MODELS
                            and len(test_accuracies_most_certain) > 0
                            and np.any(np.array(test_accuracies_most_certain) > 0.3)
                        ):
                            test_ylim_set = True
                            axacc_test.set_ylim([0.3, 0.8])
                        if (
                            args.model == "ff"
                            and len(test_accuracies_standard) > 0
                            and np.any(np.array(test_accuracies_standard) > 0.3)
                        ):
                            test_ylim_set = True
                            axacc_test.set_ylim([0.3, 0.8])
                    ...
                    axloss.plot(
                        ...
                        linewidth=1,
                        alpha=0.8,
                        label=f"Train (Aggregated): {train_losses[-1]:.4f}",
                    )
                    axloss.plot(
                        iters,
                        test_losses,
                        ...
                        linewidth=1,
                        alpha=0.8,
                        label=f"Test (Aggregated): {test_losses[-1]:.4f}",
                    )
                ...
                if is_main_process(rank) and args.model in CTM_LSTM_MODELS:
                    try:
                        ...
                        att_shape = (
                            model_module.kv_features.shape[2],
                            model_module.kv_features.shape[3],
                        )
                            ...
                            try:
                                if plot_idx < len(test_data):
                                    ...
                                    (
                                        ...
                                        posts_plot,
                                        atts_plot,
                                    ) = model_module(inputs_plot, track=True)
                                    atts_plot = atts_plot.reshape(
                                        atts_plot.shape[0],
                                        atts_plot.shape[1],
                                        -1,
                                        att_shape[0],
                                    ...
                                    make_classification_gif(
                                        img_gif,
                                        target_plot,
                                        preds_plot[0].detach().cpu().numpy(),
                                        certs_plot[0].detach().cpu().numpy(),
                                        posts_plot[:, 0],
                                        atts_plot[:, 0]
                                        if atts_plot is not None
                                        else None,
                                        class_labels,
                                    ...
                                    print(
                                        f"Warning: Plot index {plot_idx} out of range for test dataset size {len(test_data)}."
                                    )
                            except Exception as e_gif:
                                print(
                                    f"Rank 0 GIF generation failed for index {plot_idx}: {e_gif}"
                                )
            ...
            if world_size > 1:
                dist.barrier()  # Sync after evaluation block
        ...
        if (
            ...
            if args.model in CTM_LSTM_MODELS:
                save_dict["train_accuracies_most_certain"] = (
                    train_accuracies_most_certain
                )
        ...
        if world_size > 1:
            dist.barrier()  # Sync before next iteration
