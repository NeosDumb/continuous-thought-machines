...
def save_frames_to_mp4(
    ...
    if gop_size is not None and (not isinstance(gop_size, int) or gop_size <= 0):
        raise ValueError(f"gop_size must be a positive integer, got {gop_size}")
    if not isinstance(crf, int) or not (0 <= crf <= 51):
        raise ValueError(f"crf must be an integer between 0 and 51, got {crf}")
    ...
    if not isinstance(pix_fmt, str) or not re.match(r"^[a-zA-Z0-9_]+$", pix_fmt):
        raise ValueError(f"Invalid pix_fmt: {pix_fmt}")
    ...
    try:
        ...
        if len(first_frame.shape) == 3 and first_frame.shape[2] == 3:
            ...
            expected_dims = 3
            print(
                f"Info: Detected color frames (shape: {first_frame.shape}). Expecting BGR input."
            )
        ...
        if first_frame.dtype != np.uint8:
            print(
                f"Warning: First frame dtype is {first_frame.dtype}. Will attempt conversion to uint8."
            )
    ...
    if gop_size is None:
        gop_size = int(fps)
        print(
            f"Info: GOP size not specified, defaulting to {gop_size} (approx 1 keyframe/sec)."
        )
    ...
    if not isinstance(output_filename, (str, os.PathLike)):
        raise TypeError(
            f"output_filename must be a string or path-like object, got {type(output_filename)}."
        )
    ...
    if "\0" in output_filename_str:
        raise ValueError(
            "output_filename contains null bytes, which is not allowed for security reasons."
        )
    ...
    if ".." in output_filename_str.split(os.sep) or ".." in output_filename_str.split(
        ...
        raise ValueError(
            "output_filename contains directory traversal elements ('..'), which is not allowed."
        )
    ...
    command = [
        ...
        "-",
        "-vf",
        pad_filter,  # <--- ADDED VIDEO FILTER HERE
        "-c:v",
        "libx264",
        "-pix_fmt",
        pix_fmt,
    ...
    print(
        f"Parameters: FPS={fps}, Size={frame_size_str}, GOP={gop_size}, CRF={crf}, Preset={preset}"
    )
    ...
    try:
        ...
        for i, frame in enumerate(frames):
            ...
            if not isinstance(frame, np.ndarray):
                print(
                    f"Warning: Frame {i} is not a numpy array (type: {type(frame)}). Skipping."
                )
                continue
            ...
            if expected_dims == 3 and frame.shape[2] != 3:
                print(
                    f"Warning: Frame {i} is color but doesn't have 3 channels ({frame.shape}). Skipping."
                )
                continue
            ...
            if frame.dtype != np.uint8:
                try:
                    ...
                    print(
                        f"Error clipping/converting frame {i} dtype: {clip_err}. Skipping."
                    )
                    continue
            ...
            try:
                ...
                print(
                    "FFmpeg process likely terminated prematurely. Check FFmpeg errors below."
                )
                try:
        ...
        if return_code == 0:
            ...
            )  # Print stderr captured by communicate()
            print("--- End FFmpeg Output ---")
            print(
                "Review the FFmpeg error message above for details (e.g., dimension errors, parameter issues)."
            )
...
def make_classification_gif(
    ...
    figscale = 2
    with tqdm(
        total=post_activations.shape[0] + 1,
        initial=0,
        leave=False,
        position=1,
        dynamic_ncols=True,
        ...
        reducer = (
            ...
            if post_activations.shape[-1] > 2048
            else umap.UMAP(
                n_components=2,
                n_neighbors=20,
                min_dist=1,
        ...
        for stepi in np.arange(0, n_steps, 1):
            ...
            attention_interp = (
                attention_interp.flatten(1)
                - attention_interp.flatten(1).min(-1, keepdim=True)[0]
            ) / (
                attention_interp.flatten(1).max(-1, keepdim=True)[0]
                - attention_interp.flatten(1).min(-1, keepdim=True)[0]
            )
            attention_interp = attention_interp.reshape(
                n_heads, image.shape[0], image.shape[1]
            )
            ...
            for headi in range(min(8, n_heads)):
                ...
                if headi not in route_steps:
                    ...
                    A = attention_interp[headi].detach().cpu().numpy()
                    centres, areas = find_island_centers(A, threshold=0.7)
                    route_steps[headi] = route_steps[headi] + [
                        centres[np.argmax(areas)]
                    ]

            mosaic = [
                ["head_0", "head_0_overlay", "head_1", "head_1_overlay"],
            ...
            axes["certainty"].plot(
                np.arange(len(certainties_now)),
                certainties_now,
                "k-",
                linewidth=figscale * 1,
                label="1-(normalised entropy)",
            )
            for ii, (x, y) in enumerate(
                zip(np.arange(len(certainties_now)), certainties_now)
            ):
                is_correct = predictions[:, ii].argmax(-1) == target
                if is_correct:
                    axes["certainty"].axvspan(
                        ii,
                        ii + 1,
                        facecolor="limegreen",
                        edgecolor=None,
                    ...
                    axes["certainty"].axvspan(
                        ii, ii + 1, facecolor="orchid", edgecolor=None, lw=0, alpha=0.3
                    )
            axes["certainty"].plot(
                len(certainties_now) - 1,
                certainties_now[-1],
                "k.",
                markersize=figscale * 4,
            )
            ...
            axes["probabilities"].bar(
                np.arange(len(bar_heights))[::-1],
                bar_heights,
                color=np.array(colours),
                alpha=1,
            )
