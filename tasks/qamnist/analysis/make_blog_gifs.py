...
def compose_modular_expressions(input_string):
    ...
    for line in lines:
        ...
        parsed.append(
            (expr.strip(), int(mod_val), result, re.compile(rf"\b{result}\b"))
        )

    # Start with first expression as value provider
    for i in range(len(parsed) - 1):
        outer_expr, outer_mod, outer_result, outer_pattern = parsed[i]
        inner_expr, inner_mod, inner_result, inner_pattern = parsed[i + 1]

        # Replace the outer result in inner expression
        replacement = f"(({outer_expr}) mod {outer_mod})"
        new_inner_expr = outer_pattern.sub(replacement, inner_expr, count=1)

        # Update the next expression with substituted version
        parsed[i + 1] = (new_inner_expr, inner_mod, inner_result, inner_pattern)

    # Final expression
    final_expr, final_mod, final_result, _ = parsed[-1]
...
def make_qamnist_gif(
    ...
    for _ in range(3, 8):
        mosaic.append(["umap", "umap", "umap", "umap", "umap", "umap"])
    ...
    for stepi in range(n_steps):
        fig_gif, axes_gif = plt.subplot_mosaic(
            mosaic=mosaic, figsize=(31 * figscale * 8 / 4, 76 * figscale)
        )
        ...
        colors = [
            ("g" if i == this_target else ("r"))
            for i, e in enumerate(these_predictions[:, stepi])
        ]
        sort_idxs = np.arange(len(these_predictions[:, stepi]))

        # Add probability plot
        probs = softmax(these_predictions[:, stepi])
        bars_prob = axes_gif["probs"].bar(
            np.arange(len(probs)),
            probs[sort_idxs],
            color=np.array(colors)[sort_idxs],
            width=0.9,
            alpha=0.5,
        )
        ...
        for i, (bar, label) in enumerate(zip(bars_prob, class_labels)):
            ...
            axes_gif["probs"].text(
                ...
                transform=axes_gif[
                    "probs"
                ].get_xaxis_transform(),  # ← anchors to axis, not data
            )
